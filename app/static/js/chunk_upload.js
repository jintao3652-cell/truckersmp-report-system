async function uploadInChunks(file, token, metadata = {}, chunkSize = 8 * 1024 * 1024) {
  ...
  const init = await fetch("/api/v1/uploads", {method: "POST", headers: {...auth, "Content-Type": "application/json"}, body: JSON.stringify({filename: file.name, size: file.size, ...metadata})});
  if (!init.ok) throw new Error((await init.json()).error || "upload init failed");
  const session = await init.json();
  let offset = Number(localStorage.getItem(`upload:${session.id}`) || 0);
