async function uploadInChunks(file, token, metadata = {}, chunkSize = 8 * 1024 * 1024) {
  ...
  const base = (window.UPLOAD_API_BASE || "").replace(/\/$/, "");
  const options = {credentials: "include"};
  const init = await fetch(`${base}/api/v1/uploads`, {method: "POST", ...options, headers: {...auth, "Content-Type": "application/json"}, body: JSON.stringify({filename: file.name, size: file.size, resume_id: resumeId, ...metadata})});
  ...
  while (offset < file.size) {
    ...
    for (let attempt = 0; attempt < 4; attempt++) {
      try {
        response = await fetch(`${base}/api/v1/uploads/${session.id}?offset=${offset}`, {method: "PUT`, ...options, headers: auth, body: chunk});
  ...
  const done = await fetch(`${base}/api/v1/uploads/${session.id}/complete`, {method: "POST", ...options, headers: auth});
